...
class r_entropymodel(nn.Module):
    def __init__(self, input_dim=49, hidden_dim=256, output_dim=2, num_res_blocks=3):
        super(r_entropymodel, self).__init__()
    ...
    def forward(self, x, x2):
        ...
        lower = self.logistic_cdf(x2 - 0.5, mu, sigma)
        upper = self.logistic_cdf(x2 + 0.5, mu, sigma)
        pmf = (upper - lower).clamp(min=1e-9)

        return pmf, mu, sigma, x1
